...
const SyriatelCashModal = ({
  ...
  qrUrl: string;
  walletCode: string;
  onSubmitReference: (referenceNumber: string) => void | Promise<void>;
    ...
    >
      ...
      >
        <h2 className="text-lg font-bold mb-4 text-center text-slate-800 dark:text-slate-100">
          {isArabic ? "محفظة سيريتل كاش" : "Syriatel Cash Wallet"}
        </h2>
        ...
        {qrUrl ? (
          ...
          <div className="w-40 h-40 rounded border border-dashed border-slate-300 dark:border-slate-700 mb-4 flex items-center justify-center text-center text-xs text-slate-500 px-3">
            {isArabic
              ? "لم يتم ضبط صورة QR لسيريتل كاش بعد. يرجى التواصل مع الإدارة."
              : "Syriatel Cash QR is not configured yet. Please contact the admin."}
          </div>
        )}
        ...
        {step === "qr" && (
          <div className="flex gap-2 w-full mt-2">
            <button
              className="flex-1 py-2 rounded-lg font-semibold bg-amber-600 hover:bg-amber-700 text-white transition shadow-md"
              disabled={!qrUrl && !walletCode}
              onClick={() => setStep("input")}
              type="button"
            >
              {isArabic ? "تم الدفع" : "I Paid"}
            </button>
            <button
        ...
        {step === "input" && (
          <form
            ...
            onSubmit={(e) => {
              ...
              setError("");
              onSubmitReference(referenceNumber.trim());
            }}
          >
            <input
              type="text"
              ...
              placeholder={
                isArabic
                  ? "رقم المرجع من سيريتل كاش"
                  : "Syriatel reference number"
              }
...
export default SyriatelCashModal;
