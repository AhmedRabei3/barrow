...
export const GET = withMobileOrWebAuth(async (req: AuthedRequest) => {
  ...
  try {
    const latestRequest = await prisma.shamCashActivationRequest.findFirst({
    ...
    return NextResponse.json({
      ...
      message: latestRequest
        ? t("تم تحميل حالة طلب التفعيل", "Activation status loaded")
        : t(
            "لا يوجد طلب تفعيل ShamCash بعد",
            "No ShamCash activation request yet",
          ),
    });
  } catch (error) {
    const rawMessage =
      error instanceof Error
        ? error.message
        : "Failed to load ShamCash activation status";
...
export const POST = withMobileOrWebAuth(async (req: AuthedRequest) => {
  const isArabic = resolveIsArabicFromRequest(req);
  const t = (ar: string, en: string) => (isArabic ? ar : en);
  ...
  try {
    ...
    const displayName =
      req.user.name?.trim() || req.user.email?.trim() || req.user.id;
    const adminMessage = [
      t(
